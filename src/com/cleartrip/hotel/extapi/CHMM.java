...
public class CHMM extends IndiaHotels {
       ...
       // DefaultHttpClient httpclient = new DefaultHttpClient();
        ...
        try {
            HttpGet getRequest = new HttpGet(api_url() + "/hotels/1.0/search?check-in=" + putDateApi(chkinDate) + "&check-out=" + putDateApi(chkoutDate) + "&no-of-rooms="
                    + number_of_rooms + "&adults-per-room=" + adults_per_room + "&children-per-room=" + children_per_room + "&city=New+Delhi&country=IN");
    ...
    @Test( dependsOnMethods = {"CHMM_Api_QA"})
    public void Cancel_Api_QA() throws Exception {
       ...
       //System.out.println("Api CHMM Hotels TripID = " + Tripid[0]);     	
            Reporter.log("Api CHMM Hotels TripID = " + Tripid[0]);
    ...
    @Test( dependsOnMethods = {"Cancel_Api_QA"})
    public void Refund_Api_QA() throws Exception {
    	//String NeTrip = Tripid[0];
    	 // ################################# Refund API Request ############################## //
    	try {
       ...
       // System.out.println("hotelRefund "+hotelRefund);
        
        String[] refundDetails = hotelRefund.split("<refund_amount>"); 
        String[] refundDetails1 = refundDetails[1].split("</refund_amount>"); 
        float refundAmnt = Float.valueOf(refundDetails1[0]);
        Reporter.log("Refund Amount = " + refundDetails1[0]);
        //System.out.println("hotelRefund "+refundDetails1[0]);
