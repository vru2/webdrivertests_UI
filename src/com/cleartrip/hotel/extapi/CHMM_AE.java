...
public class CHMM_AE extends IndiaHotels {
    ...
    public void CHMM_AE_Api_QA() throws Exception {
        ...
        try {
            HttpGet getRequest = new HttpGet(api_url() + "/hotels/1.0/search?check-in=" + putDateApi(chkinDate) + "&check-out=" + putDateApi(chkoutDate) + "&no-of-rooms="
                    + number_of_rooms + "&adults-per-room=" + adults_per_room + "&children-per-room=" + children_per_room + "&city="+City+"&country=IN&sct=AE&scr=AED");
          ...
          //  System.out.println(hotelDetails);
            ...
            String Book = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\" ?><book-request xmlns=\"http://www.cleartrip.com/hotel/book-request\"><affiliate-txn-id>123456</affiliate-txn-id><nri>false</nri><hotel-id>"
                    ...
                    + room_type_code[0]
                    + "</room-type-code>"
                    +"<booking-country>AE</booking-country>"
                    +"<booking-currency>AED</booking-currency><booking-amount>"
                    + Cost_Price
                    + "</booking-amount>"
                    + "<customer><title>"
                    + "Mr"
            ...
            if ( api_payment_flag() ) {
                ...
                if(!bookDetails.contains("Your company's deposit account supports payments only in INR. Please change the currency of search to INR to proceed further")) {
                	Reporter.log("message doesnt contain - Your company's deposit account supports payments only in INR. Please change the currency of search to INR to proceed further");
