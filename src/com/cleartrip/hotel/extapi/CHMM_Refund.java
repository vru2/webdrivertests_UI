...
public class CHMM_Refund extends IndiaHotels {		
	String City = "Pune";
    ...
    StringBuilder refundResultbuilder = new StringBuilder();
    ...
    @Test(alwaysRun = true)
    public  void CHMM_Api_QA() throws Exception {
          ...
          //  System.out.println(hotelDetails);
           ...
           //System.out.println("Provisional_book XML :"+provisional_book);
            ...
            if (MakePaymentOnlyInQA2) {
                ...
                Tripid = getTripId[1].split("</booking-id>");
                System.out.println("API CHMM TripID MultiPax : " + Tripid[0]);
                String [ ] getconfirmation_number = bookDetails.split("<confirmation-number>");
                confirmation_number = getconfirmation_number[1].split("</confirmation-number>");
                ...
                Reporter.log("API CHMM TripID MultiPax : " + Tripid[0]);
    ...
    @Test( dependsOnMethods = {"CHMM_Api_QA"}, alwaysRun = true)
    public void Cancel_Api_QA() throws Exception {
	    	try {        
	        HttpDelete deleteRequest = new HttpDelete(api_url() + "/trips/hotel/" + Tripid[0]);
	        deleteRequest.addHeader("X-CT-API-KEY", APIKey_QA);
        ...
        refund_amount = getrefund_amount[1].split("</refund-amount>");
        ...
        System.out.println("Api CHMM Hotels TripID = " + Tripid[0]);     	
            Reporter.log("Api CHMM Refund TripID = " + Tripid[0]);
            ...
            logger.info("Api CHMM Hotels TripID = " + Tripid[0]);
    } 
    catch (ClientProtocolException e) {
        e.printStackTrace();
    }
    ...
    @Test( dependsOnMethods = {"Cancel_Api_QA"}, alwaysRun = true)
    public void Refund_Api_QA() throws Exception {
    	 // ################################# Refund API Request ############################## //
    	try {
        Reporter.log("Refund Url : "+api_url() + "/hotels/1.0/refund-info/"+Tripid[0]);        	 
        HttpGet getRefund = new HttpGet(api_url() + "/hotels/1.0/refund-info/"+Tripid[0]);
        getRefund.addHeader("X-CT-API-KEY", APIKey_QA); //8a1731a74e8016392fb9938492cf8407  
        getRefund.addHeader("X-CT-SOURCETYPE", "API");
        HttpResponse httpResponse1 = httpclient.execute(getRefund);    
        BufferedReader refundbr = new BufferedReader(new InputStreamReader((httpResponse1.getEntity().getContent())));
        String refundxml;      
        while ((refundxml = refundbr.readLine()) != null) {
        	refundResultbuilder.append(refundxml);
        }   String hotelRefund = refundResultbuilder.toString();        
        String[] refundDetails = hotelRefund.split("<refund-amount>"); 
        String[] refundDetails1 = refundDetails[1].split("</refund-amount>"); 
        float refundAmnt = Float.valueOf(refundDetails1[0]);
        Reporter.log("Refund Amount = " + refundDetails1[0]);
        if(!refund_amount[0].equals(refundDetails1[0])) {
        	Reporter.log("refund_amount[0] : "+refund_amount[0]+"refundDetails1[0] : "+refundDetails1[0]);
