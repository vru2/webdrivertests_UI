...
public class Expedia_Prod_Dom extends IndiaHotels {
        ...
        try {
          //System.out.println("Search Request Execution.....");
            HttpGet getRequest = new HttpGet(api_url() + "/hotels/1.0/search?check-in=" + putDateApi(chkinDate) + "&check-out=" + putDateApi(chkoutDate) + "&no-of-rooms="
                    + number_of_rooms + "&adults-per-room=" + adults_per_room + "&children-per-room=" + children_per_room + "&city=Mumbai&country=IN");
         ...
         // ################################# Post - Book Request ############################## //
           ...
           System.out.println("Provisional-id Expedia        = " + provisional[0]);
