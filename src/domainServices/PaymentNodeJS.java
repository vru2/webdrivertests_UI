...
public class PaymentNodeJS extends API_PaymentCommon {
	...
	public void add_GV(RemoteWebDriver driver, String amount) throws Exception {
		...
		String GV[] = getGV(1);
		//System.out.println("getGV " +getGV(1).toString());
		//Reporter.log(getGV(1));
	...
	//	textPresent_Log(driver, "Your payment is successful", 10);
		if (textPresent(driver, "Payment successful", 5)) {
			...
			logURL(driver);
			Assert.assertTrue(false);
	} else if (textPresent(driver, "Payment failed", 5)) {
			Reporter.log("Payment failed");
			logURL(driver);
	...
	// logURL(driver);
		...
		elementPresent_Time(driver, getObjectPayment("PaymentConfirmation_Msg"), 5);

		elementPresent_log(driver, getObjectPayment("PaymentConfirmation_Msg"), "", 5);
		}
