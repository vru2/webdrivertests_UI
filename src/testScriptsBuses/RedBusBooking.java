...
public class RedBusBooking extends AirCommonMethod {
	...
	@Test
	public void redBusBooking() throws Exception {

		String[] fromSet = { "Bangalore", "Delhi" };
		String[] toSet = { "Goa", "Ahmedabad" };
		boolean bookingPassed = false;
		int attempt = 0;
		boolean redBusFound = false;
		...
		do {
			...
			for (WebElement tab : tabs) {
				if (tab.getText().equalsIgnoreCase("redBus")) {
					tab.click();
					redBusFound = true;
				}
			}
			if (!redBusFound) {
				Reporter.log("Cant find red bus results. will attempt with different sector.");
				//System.out.println("Cant find red bus results. will attempt with different sector.");
				attempt++;
				continue;
			}
			...
			assertTrue("assert from in seat select modal window", fromSet[attempt].equalsIgnoreCase(driver.findElement(
					By.xpath("//div[1]/div[2]/div[2]/dl[1]/dd[1]/span[1]")).getText())); // paramBus.get("select_seat_src")
			assertTrue("assert to in seat select modal window", toSet[attempt].equalsIgnoreCase(driver.findElement(
					By.xpath("//div[1]/div[2]/div[2]/dl[1]/dd[1]/span[2]")).getText())); // paramBus.get("select_seat_dest")
			...
			Thread.sleep(5000);
			// elementnotPresent("//div[1]/div[2]/div[2]/dl[1]/dd[1]/span[1]", 300);

			driver.switchTo().window("");

			safeClick(driver, By.xpath("//*[@id='itineraryBtn']")); // . paramBus.get("bus_itinerary")

			// travellerDetails(driver, "1", "0", "0", false);
			...
			/*if ( elementVisible("//*[@id='AdultTitle1']",10) ) {
				safeSelect("//*[@id='AdultTitle1']","Mr");
				safeType("//*[@id='AdultFname1']","test");
				safeType("//*[@id='AdultLname1']","test");
				safeType("//*[@id='AdultAge1']","25");
			} */
			

			/*if ( selenium.getText("//*[@id='mobileNumber']").equals("") ) {
				safeType("//*[@id='mobileNumber']", cachedProperties.value("mobileNo"));  // cache pro
			}*/
