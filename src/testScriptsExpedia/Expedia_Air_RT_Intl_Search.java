...
public class Expedia_Air_RT_Intl_Search extends AirCommonMethod{
	...
	boolean nearbyairport=false;

	
	@DataProvider(name = "Expedia_Air_RT_Intl_Search")
	...
	@Test(dataProvider = "Expedia_Air_RT_Intl_Search")
	public void expedia_Air_RT_Intl_Search_19229(String[] origin, String[] destin,String flightPreference,String adults, String children,
		...
		//List<WebElement> we=driver.findElements(By.xpath("//div[@id='nearbyAirports']/p/label"));
		List<WebElement> we=driver.findElements(By.xpath(getValue("ExpediaNearbyAirport")));
		if(we.size()>0){
			for(int i=0;i<we.size();i++){
				String s=we.get(i).getText();
				
				Reporter.log("Nearby Airport="+s);
				//System.out.println("nearby airports are="+we.get(i).getText());
			}
		...
		//} while (!flightCountFailure);
			assertTrue("SRP didnt get loaded ", (flightCountFailure));
			assertTrue("Nearby airports didnt found",nearbyairport);
