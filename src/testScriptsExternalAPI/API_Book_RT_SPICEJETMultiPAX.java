...
public class API_Book_RT_SPICEJETMultiPAX extends APIServices{
	
	DefaultHttpClient clientSearch = null;
	ArrayListMultimap<String, String> fareKeys=ArrayListMultimap.create();
	String payLoadFileName = "PayloadRTMultiPAX.txt";
	...
	public void retailOneAdult_354(String from,String to,String carrier,String aCount,String cCount,String iCount) throws Exception{
	...
	//System.out.println(s);
		...
		do {
			String key,key1;
			if(fareKeys.size()>0&&fareKeys1.size()>0){
				 key = fareKeys.get(i);
			//System.out.println(key);
				 key1=fareKeys1.get(i);}
			else{
				  i++;
				  continue;
			}
...
//System.out.println("return fare key="+key1);
			...
			String post = apiPayload.value("createItineraryDOMRTMultiPAX");
			Random r = new Random();
	 		char c = (char) (r.nextInt(26) + 'a');
	 		//System.out.println(post.replaceFirst("#Key",key));
	 		//System.out.println(post.replaceFirst("#rtKey",key1));
			String postString1 = post.replaceFirst("#key",key)
					...
					.replaceFirst("#rtkey",key1)
					.replaceFirst("#from",from)
					.replaceFirst("#to",to).replaceFirst("#rtFlightNo",rtFlightNo).replaceFirst("#carrier",carrier)
					.replaceFirst("#returndate", returndate).replaceAll("#Jivan","Jivann"+c).replaceAll("#Kotian","Kotiann"+c).replaceAll("#Test","Test"+c);
//System.out.println(postString1);
			HttpResponse itinenaryResponse = APIUtils.postApi(getBaseUrl() + "/air/1.0/itinerary/create", headers, postString1,
					200);
		...
		/*
		//System.out.println("Api_Book_RT_SPICEJET");
	    ...
	    HttpGet get = new HttpGet(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/search?from="+from+"&to="+to+"&depart-date="+onwarddate+"&return-date="+returndate+"&adults="+aCount+"&children="+cCount+"&infants="+iCount+"&carrier="+carrier+"&cabin-type=Economy"));
		...
		for(int b=0;b<returnSolutions.length();b++){
			fareKeys.put(String.valueOf(b),returnSolutions.getJSONObject(b).getJSONObject("pax-pricing-info-list").getJSONArray("pax-pricing-info").getJSONObject(0).getJSONObject("pricing-info-list").getJSONObject("pricing-info").get("fare-key").toString());
		}
	   ...
	   //   //System.out.println(postString);
		//		//System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/create");
		    ...
		    String responseString = EntityUtils.toString(entityIti, "UTF-8");
		  	 ...
		  	 //Reporter.log(itineraryId.toString());
		  	if ((common.value("makePayment").equals("true"))) {
	    	 ...
	    	 //   try{
			        ...
			        bookCall1.setHeader("X-CT-API-KEY","g9s45bsammqggtczpz3kj3qk");
	...
	String responseString = EntityUtils.toString(entityBook1, "UTF-8");
	    	    ...
	    	    Reporter.log(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itineraryId.getJSONObject("itinerary").get("itinerary-id"),true);
	    	   // System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itineraryId.getJSONObject("itinerary").get("itinerary-id"));
		        HttpPost bookCall = new HttpPost(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itineraryId.getJSONObject("itinerary").get("itinerary-id")));
