...
public class API_IXIGO_INTL_Indigo_Booking extends CommonUtils{
	...
	String TotalFare;
	HttpPost bookCall;
	ArrayListMultimap<String, String> fareKeys1=ArrayListMultimap.create();
	//public RemoteWebDriver driver = null;
	...
	System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/search?from="+from+"&to="+to+"&depart-date="+onwarddate+"&adults="+aCount+"&children="+cCount+"&infants=0&carrier="+carrier+"&cabin-type=Economy");
		...
		int x=1;
		for(int i=1;i<fareKeys.size();i++){
			
		//System.out.println(fareKeys.get(String.valueOf(i)).toString().split("retail")[1].split("_")[1]);
			try{
			if(fareKeys.get(String.valueOf(i)).toString().split("retail")[1].split("_")[1].equalsIgnoreCase("9W")){
				
				fareKeys1.put(String.valueOf(x),fareKeys.get(String.valueOf(i)).toString().replace("[","").replace("]",""));
				System.out.println(fareKeys1.get(String.valueOf(i)));
				x++;
			}
			}
			catch(Exception e){
		...
		//Test:do{
			...
			String key1=fareKeys1.get(String.valueOf(i)).toString().replace("[","").replace("]","");
			//System.out.println(key1);
			System.out.println("-----"+fareKeys1.get(String.valueOf(i)).toString());
			String flightno = fareKeys1.get(String.valueOf(i)).toString().split(carrier.trim())[1].split("_")[1].trim();
