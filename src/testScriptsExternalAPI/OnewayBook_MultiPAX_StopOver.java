...
public class OnewayBook_MultiPAX_StopOver extends APIServices{
	...
	public void retailOneAdult_357(String[] from,String[] to,String[] carrier,String aCount,String cCount,String iCount) throws Exception{
...
//System.out.println("--------------"+bookResponseString);
		...
		////System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/search?from="+from+"&to="+to+"&depart-date="+onwarddate+"&adults="+aCount+"&children="+cCount+"&infants=0&carrier="+carrier+"&cabin-type=Economy");
		 HttpGet get = new HttpGet(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/search?from="+from+"&to="+to+"&depart-date="+onwarddate+"&adults="+aCount+"&children="+cCount+"&infants=0&carrier="+carrier+"&cabin-type=Economy"));
		  ...
		  //  getdepositaccountid(common.value("AccountID"),"Book.txt");
			...
			HttpEntity entity = response.getEntity();
			 ...
			 StringBuffer sb1 = new StringBuffer();
				...
				for(int a=0;a<onwardSolutions.length();a++){
					...
					//System.out.println(onwardSolutions.getJSONObject(a).getJSONObject("flights").getJSONObject("flight").getJSONObject("segments").toString());
			...
			//if(String.valueOf(onwardSolutions.getJSONObject(a).getJSONObject("flights").getJSONObject("flight").getJSONObject("segments").toString().charAt(1)).equalsIgnoreCase("[")){
					try{
					...
					fareKeys.put(String.valueOf(b),onwardSolutions.getJSONObject(a).getJSONObject("pax-pricing-info-list").getJSONArray("pax-pricing-info").getJSONObject(0).getJSONObject("pricing-info-list").getJSONObject("pricing-info").get("fare-key").toString());
					b++;
					}
					}
					catch(Exception e){
	    ...
	    do{
	    	...
	    	// System.out.println("key============"+key);
	    	 ...
	    	 //  System.out.println(flightno);
	  	    ...
	  	    // //System.out.println("itinerary="+itinenaryId);
				...
				// //System.out.println("checking error message="+itinenaryId.getJSONObject("error").getString("error-message"));
				 ...
				 ////System.out.println(message[1].equalsIgnoreCase(" please pick a different fare"));
		  	     ...
		  	     if(message[1].trim().equalsIgnoreCase("please pick a different fare")){
		  	       ...
		  	       ////System.out.println("new fare="+priceChange[15]);
		  	       String mfarekey1=farekey.replaceAll(oldfare[oldfare.length-1],priceChange[15]);
		  	       String mfarekey=mfarekey1.replace(".","");
		  	       ////System.out.println(mfarekey);
		  	   String postString1="<?xml version=\"1.0\" encoding=\"UTF-8\"?><itinerary>  <cabin-type>E</cabin-type>  <flights>    <flight-spec>      <fare-key>"+mfarekey+"</fare-key>      <segments>        <segment-spec>          <departure-airport>"+from+"</departure-airport>          <arrival-airport>"+to+"</arrival-airport>          <flight-number>"+flightno+"</flight-number>          <airline>"+carrier+"</airline>          <departure-date>"+onwarddate+"</departure-date>        </segment-spec>      </segments>    </flight-spec>  </flights>  <pax-info-list>    <pax-info>      <title>Mr</title>      <first-name>Jivan</first-name>      <last-name>Kottian</last-name>      <type>ADT</type>      <id-number>Grant</id-number>      <date-of-birth>1990-05-05</date-of-birth>    </pax-info>    <pax-info>      <title>Mstr</title>      <first-name>Test</first-name>      <last-name>Test</last-name>      <type>CHD</type>      <date-of-birth>2010-05-05</date-of-birth>    </pax-info>    <pax-info>      <title>Mstr</title>      <first-name>Test</first-name>      <last-name>Test</last-name>      <type>INF</type>      <date-of-birth>2016-05-05</date-of-birth>    </pax-info>  </pax-info-list>  <contact-detail>    <title>Mr</title>    <first-name>Test</first-name>    <last-name>Booking</last-name>    <email>nfsfrank@hotmail.com</email>    <address>Unit No 001, Ground Floor, DTC Bldg, Sitaram mills compound, N.M.joshi Marg, Lower parel (E)</address>    <mobile>919844000000</mobile>    <landline>02240554000</landline>    <city-name>Mumbai</city-name>    <state-name>Maharashtra</state-name>    <country-name>India</country-name>    <pin-code>400011</pin-code>  </contact-detail></itinerary>";
		  	    	 FileInputStream fis1 = new FileInputStream("src\\testScriptsExternalAPICommon\\"+payLoadFileName);
			    	 ...
			    	 //  //System.out.println(postString1);
			    	   HttpPost itinenaryCall1 = new HttpPost(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/create"));
			    	   StringEntity params1 = new StringEntity(postString1);
			    	   itinenaryCall1.setEntity(params1);
			    	   
			    	   itinenaryCall1.setHeader("X-CT-API-KEY","g9s45bsammqggtczpz3kj3qk");
	       ...
	       Reporter.log("sucess message=="+docBook1.getElementsByTagName("succes-message").item(0).getTextContent());
            ...
            //System.out.println("status="+hitStatus);
    	    	//System.out.println(priceChange1);
    	    	if(!priceChange1){
    	    ////System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itinenaryId.getJSONObject("itinerary").get("itinerary-id"));
	         bookCall = new HttpPost(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itinenaryId.getJSONObject("itinerary").get("itinerary-id")));
    	    	}
    	    	else{
    	    	//	//System.out.println(itinenaryId1.getJSONObject("itinerary").get("itinerary-id"));
    	    		////System.out.println(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itinenaryId1.getJSONObject("itinerary").get("itinerary-id"));
    		         bookCall = new HttpPost(new URI(common.value("protocol")+"://"+common.value("Environment")+".cleartrip.com/air/1.0/itinerary/book/"+itinenaryId1.getJSONObject("itinerary").get("itinerary-id")));
    	    	}
	    ...
	    String responseString = EntityUtils.toString(entityBook, "UTF-8");
	       ...
	       StringBuffer sb11 = new StringBuffer();
			...
			//System.out.println(jsonObject1);
	  	     if(itinenaryResponse!=null|| itinenaryResponse1!=null){
...
*/}
	...
	public static Object[][] oneAdultDp(){
		...
		return new Object[][]{{origin,destination,carrier,"1","1","1"},
