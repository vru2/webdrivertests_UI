...
public class B2CAirOWGDSDomViaParSectorFullRefundAPI extends HQ {
	...
	public void airOWGDSDomViaParSectorFullRefundAPI_134() throws Exception {

		boolean flagCancellation = false;
		boolean flagFareRules = true;
		String refundMethod = "Full Refund";

		Reporter.log("Test case " + this.getClass() + " started");
		System.out.println("Test case " + this.getClass() + " started");

		tripId = getAPITripId(domain, "B2CAirOWGDSDomViaParSectorFullRefundAPI", "email_id", "user_id");
		...
		if (flagCancellation && !(status.contains("Booking Failed")) && status != null) {
			...
			cancellationTripPartialSectorHQ(driver, "partial via trip cancellation");// full pax partial via segment
																						// cancellation
			...
			if (cancelSuccess) {
				...
				if (cancelSuccess) {
					...
					if (flagCancellation) {
						cancellationTripProcessStartHQ(driver);
						cancellationTripPartialSectorHQ(driver, "partial via trip cancellation");
