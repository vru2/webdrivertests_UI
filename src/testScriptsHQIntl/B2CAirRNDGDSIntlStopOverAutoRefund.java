...
public class B2CAirRNDGDSIntlStopOverAutoRefund extends AirCommonMethod {
	...
	/*
	 * @DataProvider(name = "B2CAirRNDGDSIntlStopOverAutoRefund") public static Object[][] B2CAirRNDGDSIntlStopOverAutoRefund() {
	 * String origin[] = { "DEL", "CMB", "BOM" }; String destination[] = { "DXB", "MAA", "DXB" }; return new Object[][] { {
	 * origin, destination, "Flights", "RoundTrip", "Air India", "Non Direct", "1", "1", "1", "credit card", "stopover" false,
	...
	@Test(groups="Regression",dataProviderClass = HQDataProvider.class, dataProvider = "B2CAirRNDGDSIntlStopOverAutoRefund")
	public void airRNDGDSIntlStopOverAutoRefund(String[] fromSet, String[] toSet, String app, String tripType,
		...
		do {
			...
			if (!flightPreference.equals("")) {
				warningFound = filterFlightsByPreferedAirlineReturn(driver, flightPreference, 0);
				if (warningFound) {
					attempt++;
					continue;
				}
			...
			/*
			 * WebElement pickedFlight = selectRoundTripFlightIntl(driver, flightSegments); if (pickedFlight == null) { attempt++;
			 * continue; } else {
		...
		if (common.value("HQFlowFlag").equals("true") && common.value("makePayment").equals("true")) {
			...
			if (flagCancellation && !(status.contains("Booking Failed")) && status != null) {
				...
				hq.cancellationTripPartialSectorHQ(driver, "partial via trip cancellation");
