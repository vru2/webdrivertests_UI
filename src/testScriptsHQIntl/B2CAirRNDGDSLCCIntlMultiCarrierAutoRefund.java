...
public class B2CAirRNDGDSLCCIntlMultiCarrierAutoRefund extends AirCommonMethod {
	...
	/*
	 * @DataProvider(name = "B2CAirRNDGDSIntlMultiCarrierAutoRefund") public static Object[][]
	 * B2CAirRNDGDSIntlMultiCarrierAutoRefund() { String origin[] = { "DEL", "CMB", "BOM" }; String destination[] = { "DXB",
	 * "MAA", "DXB" }; return new Object[][] { { origin, destination, "Flights", "RoundTrip", "Non Direct", "1", "1", "1",
	 * "credit card", "MultiCarrier", false, "Auto Refund", false, "gds" } }; }
	 */
	...
	@Test(groups="Regression",dataProviderClass = HQDataProvider.class, dataProvider = "B2CAirRNDGDSIntlMultiCarrierAutoRefund")
	public void airRNDGDSIntlMultiCarrierAutoRefund(String[] fromSet, String[] toSet, String app, String tripType,
			String flightFilterType, String adults, String children, String infants, String paymentMethod, String flightSegments,
			boolean insuranceRequired, String refundMethod, boolean sector, String flight_type) throws Exception {/*
...
Reporter.log("Test case " + this.getClass() + " started");
		...
		do {
			...
			selectRoundTripFlight(driver, flightSegments);// TODO: check working
			...
			travellerDetails(driver, adults, children, infants, true);
		...
		if (common.value("HQFlowFlag").equals("true") && common.value("makePayment").equals("true")) {
			...
			hq.signInHQ(driver);
			hq.confirmTripLoad(driver, tripId);
			...
			if (flagCancellation && !(status.contains("Booking Failed")) && status != null) {
				...
				hq.cancellationTripPartialPaxHQ(driver, "all");// full pax partial cancellation
				...
				if (cancelSuccess) {
					...
					hq.confirmTripLoad(driver, tripId, domain);
					cancelSuccess = hq.checkIfThisStatus(driver, "Cancelled");
					if (cancelSuccess) {
				...
				hq.cancellationQueueProcess(driver, refundMethod, tripId);
				...
				hq.confirmTripLoad(driver, tripId);
				boolean fFlag = hq.processForwarded(driver, tripId, 5);
				if (fFlag)
				...
				Reporter.log(this.getClass() + " = " + "Error occurred. cancel link:" + flagCancellation + " status:" + status + " fare rules: "
						+ flagFareRules);
				//System.out.println(this.getClass() + " = " + "Error occurred. cancel link:" + flagCancellation + " status:" + status + " fare rules: "
						+ flagFareRules);
				assertTrue("Failure!", false);
			}
		} else {
