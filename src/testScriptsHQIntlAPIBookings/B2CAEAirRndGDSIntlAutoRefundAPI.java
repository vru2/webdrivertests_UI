...
public class B2CAEAirRndGDSIntlAutoRefundAPI extends HQ {
	...
	public void aEAirRndGDSIntlAutoRefundAPI_152() throws Exception {
		...
		tripId = getAPITripId(domain, "B2CAEAirRndGDSIntlAutoRefundAPI", "email_id", "user_id");
		...
		if (flagCancellation && !(status.contains("Booking Failed")) && status != null) {
			...
			cancellationTripPartialPaxHQ(driver, "all");// full pax full sector cancellation
			...
			if (cancelSuccess) {
				...
				if (cancelSuccess) {
					...
					if (flagCancellation) {
						cancellationTripProcessStartHQ(driver);
						cancellationTripPartialPaxHQ(driver, "all");
