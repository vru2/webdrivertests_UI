package testScriptsHQIntlAPIBookings;
...
public class B2CAirRNDGDSIntlPartialSegAutoRefundAPI extends HQ {
	...
	public void airRNDGDSIntlPartialSegAutoRefund_151() throws Exception {
		...
		tripId = getAPITripId(domain, "B2CAirRNDGDSIntlPartialSegAutoRefundAPI", "email_id", "user_id");
		driver.get(getBaseUrl(domain) + "/hq");
		signInHQ(driver);
		confirmTripLoad(driver, tripId, domain);
		String status = getBookingStatus(driver);
		...
		if (flagCancellation && !(status.contains("Booking Failed")) && status != null) {
			...
			cancellationTripPartialSectorHQ(driver, "partial round trip cancellation");// full pax partial round trip
																						// cancellation
