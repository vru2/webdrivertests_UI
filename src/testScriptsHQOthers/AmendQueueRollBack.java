...
public class AmendQueueRollBack extends AirCommonMethod {
	...
	public void amendQueueRollBack() throws Exception {
		...
		if (isPendingAmendment) {
			...
			hq.processAmendQueueRollBack(driver, tripId);
			hq.confirmTripLoad(driver, tripId, domain);
			isPendingAmendment = hq.checkIfThisStatusForAtleastOneSegment(driver, "Pending (Amendment)");

			if (!isPendingAmendment) {
				// System.out.println("TripId " + tripId + " - RollBacked Successfully");
				Reporter.log("TripId " + tripId + " - RollBacked Successfully");
			} else {
