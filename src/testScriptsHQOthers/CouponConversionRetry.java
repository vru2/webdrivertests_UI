...
public class CouponConversionRetry extends AirCommonMethod {
	...
	/*
	 * @DataProvider(name = "CouponConversionConvert") public static Object[][] CouponConversionConvert() { String[] origin = {
	...
	public void couponConversionRetry(String[] fromSet, String[] toSet, String app, String tripType, String flight_type,
		...
		if (common.value("HQFlowFlag").equals("true") && common.value("makePayment").equals("true")) {
			...
			if (status.equals("Hold (Airline Coupon)")) {
				hq.processCouponConversion(driver, tripId, "retry");
				assertTrue("Retry did not happen. Error!", driver.findElementByXPath("//*[@id='Search']//td/div[2]/span")
						.getText().equalsIgnoreCase("Retried"));
				Thread.sleep(10000);
				hq.confirmTripLoad(driver, tripId, domain);
				status = hq.getBookingStatus(driver);
			} else {
