...
public class OfflineConversionRefund extends AirCommonMethod {
	...
	public void offlineConversionRefund() throws Exception {
		...
		if (isBookingFailed) {
			...
			hq.processOfflineConversionRefund(driver, tripId);
			hq.confirmTripLoad(driver, tripId, domain);

			boolean refunded = isElementPresent(driver, getObject("Air_HQ_Trip_Details_Refunds_Tab"));

			if (refunded) {
				// System.out.println("TripId " + tripId + " - Refunded Successfully");
				Reporter.log("TripId " + tripId + " - Refunded Successfully");
			} else {
				hq.confirmTripLoad(driver, tripId, domain);
				refunded = isElementPresent(driver, getObject("Air_HQ_Trip_Details_Refunds_Tab"));
				if (refunded) {
					// System.out.println("TripId " + tripId + " - Refunded Successfully");
					Reporter.log("TripId " + tripId + " - Refunded Successfully");
				} else {
					// System.out.println("TripId " + tripId + " - Refund attempted, but failed.");
					Reporter.log("TripId " + tripId + " - Refund attempted, but failed.");
					assertTrue("Failure!", false);
				}
			}
		} else {
