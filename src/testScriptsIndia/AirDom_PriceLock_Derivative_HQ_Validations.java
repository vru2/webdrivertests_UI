...
public class AirDom_PriceLock_Derivative_HQ_Validations extends AirCommonMethod {
	...
	public void Air_PriceLock_Derivative_HQ_Validations (String[] origin, String[] destination, String tripType, String flight_type,
		...
		waitElement(driver, getObject("AirCom_PriceLock_HqTrips_Yellow_Banner"), 50);
		String lockType = getText(driver, getObject("AirCom_PriceLock_HqTrips_Yellow_Banner")); //Derivative
		boolean checkLockType=false;
		if (lockType.contains("Derivative")){
			Reporter.log(tripId+ ": Lock Type verified as 'Derivative' in HQ-Trips");
			System.out.println(tripId+ ": Lock Type verified as 'Derivative' in HQ-Trips");
		...
		Assert.assertTrue(checkLockType, "PriceLock trip is not of Derivative lock type");
		Reporter.log(tripId+ ": Lock Type 'Derivative' - 'AirLine PNR' and 'Ticketing' not done as expected.");
