...
public class AirDom_PriceLock_RT_Booking extends AirCommonMethod{
	...
	public void Air_PriceLock_RT_Booking(String[] origin, String[] destination, String tripType, String flight_type,
		...
		do {
			...
			airCom_HomepageSearch_RoundTrip_Pricelock(driver, origin[attempt], destination[attempt], adults, children, infants, flightPreference[attempt], attempt);

			Reporter.log("Search URL is : " + driver.getCurrentUrl()); // capture itinerary
			System.out.println(driver.getCurrentUrl());

			flightCountFailure = waitForElement(driver,90,getObject("AirCom_SRP_RoundTrip_BookButton"));
			...
			while(!gotLock && priceLockAttempt < 3){
				if (elementPresent(driver, getObject("AirCom_BookStep_PriceLock_rBtn"))){
					...
					System.out.println("refreshing to get lock");
			...
			if (reachedPaymentStep) {
				...
				pgFeeString=pgFeeString.replaceAll(",","");
				...
				/*System.out.println("bookStepLockfee - "+bookStepLockfee);
				System.out.println("bookStepBalance - "+bookStepBalance);
				System.out.println("lockfeeInPayBlock - "+lockfeeInPayBlock);
				System.out.println("balanceInPayBlock - "+balanceInPayBlock);
			...
			//System.out.println("PriceLock Booking Status from Accounts "+ BookStatus);
			//assertTrue("Status of the booking is NOT in HOLD", BookStatus.equalsIgnoreCase("ON HOLD"));
			boolean printstatus = BookStatus.contains("HOLD");
