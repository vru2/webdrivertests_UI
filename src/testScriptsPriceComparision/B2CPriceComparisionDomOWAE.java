...
public class B2CPriceComparisionDomOWAE extends AirCommonMethod {
	...
	public void priceComparisionDomOW(String[] origin,String[] destination) throws Exception {
		...
		HashMap<String, String> prodInfo = null;
			...
			if (prodClass.failureFlag) {
				...
				prodClass = new GetSRPInfo("http://www.cleartrip.com", driver1, origin[attempt1],destination[attempt1]);
			...
			do{
				if(attempt>0){
					//System.out.println("due to value mismatch refreshing for 10 seconds and fetching flight count");
					Reporter.log("due to value mismatch refreshing for 10 seconds and fetching flight count");
	...
	public class GetSRPInfo implements Runnable {
		...
		void captureAllValues() throws Exception {
			...
			if(waitElement(driver,By.xpath("//li[@class='allFlights']"),1)){
				...
				////System.out.println("flightname="+we.get(0).getText());
				info.put("flightName",we2.get(0).getText().trim());
			
			// info.put("price",
			// driver.findElement(By.xpath(".//li[1]/table/tbody[2]/tr[1]/th[6]/div/label[1]/span")).getText().trim());
			try{
			info.put("price", driver.findElement(By.xpath("//li[1]/table/tbody/tr[1]/th[6]")).getText().trim());

			}
			catch(Exception e){
			List<WebElement> we1=driver.findElements(By.xpath("//table/tbody/tr/th"));
			////System.out.println("price=="+we1.get(5).getText());
			info.put("price", we1.get(5).getText().trim());
			}
			List<WebElement> we3=driver.findElements(By.xpath("//tr/td/ul/li"));
			test1:for(int i=0;i<we3.size();i++){
			
			if(we3.get(i).getText().equalsIgnoreCase("Flight itinerary")){
				//System.out.println("checking link="+we3.get(i).getText());
				we3.get(i).click();
				break test1;
			}
			...
			//safeClick(driver, By.xpath("//li[1]/table/tbody[2]/tr[3]/td/ul/li[1]/a"));ecking the link
			Thread.sleep(1000);
			...
			info.put("flightNo", driver.findElement(By.xpath("//li[1]/div/ul/li[1]/div/ul/li[1]/div[2]/small[1]")).getText()
				...
				////System.out.println("flightNo=========="+driver.findElement(By.xpath("//small[@class='flightNumber']")).getText());
				info.put("flightNo",driver.findElement(By.xpath("//small[@class='flightNumber']")).getText()
						.trim());
			}
